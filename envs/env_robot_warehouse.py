...
from .robot_warehouse.env import RobotWarehouse

from .config_robot_warehouse import get_config, get_robot_warehouse_config
...
def make_eval_env(all_args, n_threads=1):
    def get_env_fn(rank):
        def init_env():
            if all_args.env_name == "robot_warehouse":
                ...
                # env = RobotWarehouse(shelf_columns, column_height, shelf_rows, 
...
class Env:
    def __init__(self, n_threads=1):
        parser = get_config()
        parser_rware = get_robot_warehouse_config()
